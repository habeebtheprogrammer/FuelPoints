...
NETWORK FIX (NEW):
  ✅ Does NOT drop the EPS connection just because the socket is idle.
  ✅ Treats socket.timeout as normal and keeps waiting.
  ✅ Treats empty recv (b"") as a real disconnect and exits cleanly.
  ✅ Optional "dead connection" detection after prolonged silence (like your working GUI).
...
class EdgeAgentWorker(QThread):
    def __init__(self, config: dict, signals: EdgeAgentSignals):
        ...
        # ---- Network idle handling (NEW, but safe defaults)
        self.SOCKET_TIMEOUT_SECONDS = 5.0
        self.MAX_SILENT_SECONDS = 10**9  # if EPS sends nothing for 60s, consider connection dead (matches your working GUI style)
    ...
    def recv_exact(self, conn: socket.socket, n: int) -> bytes:
        """
        IMPORTANT:
          - If peer closes: recv() returns b"" -> we return b"".
          - If idle: socket.timeout is raised (handled by caller).
        """
        buf = b""
        while len(buf) < n:
  ...
  {self.resp_header(pos_seq, loy_seq, iface_ver)}
    ...
    # ------------------ (GetRewardsResponse + FinalizeRewardsResponse unchanged logic) ------------------
    # NOTE: To keep your request ("everything else the same"), I am not changing
    # any punch/promo/redemption logic below. Only the network/client loop is fixed.
  ...
  {self.resp_header(pos_seq, loy_seq, iface_ver)}
    ...
    def handle_client(self, conn: socket.socket, addr):
        """
        NETWORK FIX APPLIED HERE:
          - socket.timeout is treated as normal idle -> keep waiting
          - empty recv (b"") means peer closed -> break & cleanup
          - optional prolonged silence detection -> break after MAX_SILENT_SECONDS
        """
        ...
        # Optional: Windows keepalive tuning (safe if unsupported)
        try:
        ...
        silent_ticks = 0
        max_silent_ticks = max(1, int(self.MAX_SILENT_SECONDS / max(1, int(self.SOCKET_TIMEOUT_SECONDS))))
        ...
        try:
            while self.running:
                try:
                    ...
                    # Normal idle: keep connection alive
                    silent_ticks += 1
                    if silent_ticks >= max_silent_ticks:
                        self.log(f"⚠ EPS silent for ~{self.MAX_SILENT_SECONDS}s; closing {peer}")
                        break
                    continue
                ...
                # If peer closed socket, recv_frame returns b""
                if not frame:
                    self.log(f"🔌 EPS closed connection: {peer}")
                    break
  ...
  {self.resp_header(pos_seq, loy_seq, iface_ver)}
            ...
            # common Windows disconnect errors
            if getattr(e, "errno", None) in (10053, 10054, 10057):
                pass
            else:
