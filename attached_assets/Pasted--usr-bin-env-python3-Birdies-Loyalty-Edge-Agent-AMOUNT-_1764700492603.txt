#!/usr/bin/env python3
"""
Birdies Loyalty Edge Agent - AMOUNT OFF SUPPORT
-----------------------------------------------
...
- Points Redemption (stacks on top of promotions)
- Best-price logic (picks best promo between POS and loyalty)
"""
...
def build_promotion_rewards_xml(items: list, promotions: list) -> tuple:
    ...
    - Multi-Pack: Uses newPrice method (e.g., 2 for $5 = $2.50 each)
    - Amount Off: Uses newPrice method with split discount (e.g., $1.80 off 2 = $0.90 off each)
    
    BEST-PRICE LOGIC: Only applies the single best promotion per UPC (lowest per-unit price).
    
    Returns: (list of XML strings, list of actually applied promotions with calculated discounts)
    """
    if not promotions:
        return [], []
    ...
    # BEST-PRICE SELECTION: Group promotions by (UPC, promotionId) to avoid stacking different promotions
    # but allow multiple bundles of the same promotion
    upc_promo_groups = {}
    for promo in promotions:
        upc = promo.get("upc", "")
        ...
        if group_key not in upc_promo_groups:
            ...
            # Same promotion, accumulate bundle count
            upc_promo_groups[group_key]["total_bundle_count"] += bundle_count
    
    # Now select only the best promotion per UPC (lowest per-unit price)
    upc_to_best_promo = {}
    for (upc, promo_id), group_data in upc_promo_groups.items():
        ...
        if upc not in upc_to_best_promo or per_unit_price < upc_to_best_promo[upc]["per_unit_price"]:
            ...
            log(f"🏆 Best promo for UPC {upc}: {group_data['promo'].get('description', 'N/A')} @ ${per_unit_price:.4f}/unit (bundles: {group_data['total_bundle_count']})")
    
    # Now apply only the best promotion for each UPC
    add_rewards = []
    applied_promotions = []  # Track actually applied promotions with calculated discounts
    promo_counter = 1
    
    for upc, best_data in upc_to_best_promo.items():
        promo = best_data["promo"]
        per_unit_new_price = best_data["per_unit_price"]
        total_bundle_count = best_data["total_bundle_count"]  # Use accumulated count
        
        promo_qty = int(promo.get("quantity", 2))
        discount_type = promo.get("discountType", "multipack")
        promo_name = promo.get("name") or promo.get("itemGroupName") or "Promo"
        display_name = promo_name[:24]  # Receipt has 24-char limit
        ...
        log(f"📍 Applying BEST {discount_type} promo: {promo.get('description', 'N/A')}")
        log(f"   Per-unit new price: ${per_unit_new_price:.4f}")
        
        # Distribute discount across lines with this UPC
        remaining_units = total_promo_units
        total_discount_for_promo = 0.0  # Track the actual discount applied
        ...
        for item in matching_lines:
            ...
            if current_price <= per_unit_new_price:
                log(f"   Skipping line {item['line_no']}: current ${current_price:.2f} <= new ${per_unit_new_price:.2f}")
                continue
            ...
            # Calculate actual discount for this line
            ...
            log(f"   ✓ Applying to line {item['line_no']}: {take_qty} units @ ${per_unit_new_price:.4f} each (${line_discount:.2f} off)")
    ...
    <AddReward>
      ...
      <RewardReceiptDescLong>{display_name}</RewardReceiptDescLong>
        ...
        # Record the actually applied promotion with CALCULATED discount
        if total_discount_for_promo > 0:
            applied_promo = promo.copy()
            applied_promo["discount"] = total_discount_for_promo  # Use calculated discount
            applied_promo["name"] = display_name
            applied_promotions.append(applied_promo)
        
        promo_counter += 1
    
    return add_rewards, applied_promotions
...
def build_get_rewards_response(root: ET.Element) -> str:
    ...
    global last_promotions_applied
    if not loyalty_id and not phone:
        ...
        last_promotions_applied = applied_promos  # Store ONLY applied promotions for finalize
