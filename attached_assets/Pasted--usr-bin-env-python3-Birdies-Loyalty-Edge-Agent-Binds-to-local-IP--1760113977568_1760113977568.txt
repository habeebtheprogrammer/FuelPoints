...
<GetRewardsResponse>
  ...
  <RewardActions>
        ...
        # Calculate redemption recommendation
        # Extract subtotal from transaction if available
        subtotal = 0.0
        transaction_elem = root.find(".//TransactionData")
        if transaction_elem:
            total_elem = transaction_elem.find(".//Total")
            if total_elem is not None and total_elem.text:
                try:
                    subtotal = float(total_elem.text)
                except:
                    pass
