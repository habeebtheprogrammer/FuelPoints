...
def build_promotion_rewards_xml_eps(items: list, promotions: list) -> tuple:
    """
    ...
    Returns: (list of AddReward XML strings, list of actually applied promotions with calculated discounts)
    """
    if not promotions:
        return [], []
    ...
    add_rewards = []
    applied_promotions = []  # Track which promotions were actually applied with calculated discounts
    ...
    <ns3:AddReward>
      ...
      <ns3:RewardReceiptDescLong>{display_name}</ns3:RewardReceiptDescLong>
            ...
            # Record the actually applied promotion with the CALCULATED discount (not original API discount)
...
def build_get_rewards_response(root: ET.Element) -> str:
    ...
    if not loyalty_id and not phone:
        ...
        promo_rewards, applied_promos = build_promotion_rewards_xml_eps(items, promotions)
