    ...
    if sess.customer:
        ...
        for f in intended_confirmed:
            if f.punch_card_id and f.punch_card_id not in redeemed_ids:
                try:
                    redeem_result = redeem_punch_reward(customer_id, f.punch_card_id, txn_id)
...
def build_cancel_redemption_response(root: ET.Element) -> str:
    ...
    if sess and reward_id:
        ...
        log(f"[Session] CancelRedemption: added {reward_id} to cancelled list")
    ...
    try:
        ...
        while True:
            ...
            if tag == "GetLoyaltyOnlineStatusRequest":
                ...
                # Ignore for session resets (fuel/outside sales overlap)
