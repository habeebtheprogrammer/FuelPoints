#!/usr/bin/env python3
"""
Birdies Loyalty Edge Agent - COMBINED REWARDS + PUNCH CARDS
============================================================
Gilbarco Passport POSLOYALTY integration with:
...
OPTION A LOGIC (Promo Overrides Punch System):
----------------------------------------------
If an item receives a promotional discount:
  1. It does NOT earn punches
  2. It CANNOT have a punch reward redeemed on it

This is the cleanest rule: "promo and punch are mutually exclusive per line item."

CONCURRENCY SAFETY:
-------------------
- POS_SINGLETON_LOCK ensures only one transaction is processed at a time
- State is cleared in finally block to prevent dirty state on disconnect
- Transaction IDs are consistent across all backend calls

HEARTBEAT LOGIC:
----------------
- Heartbeat is ONLY sent when GetLoyaltyOnlineStatusRequest is received
- This proves true end-to-end connectivity (POS → Edge → Backend)
- If POS is blocked by firewall, backend correctly shows "offline"
"""
...
# Network Settings
HOST = "10.96.10.175"          # Edge agent's IP on Passport network
...
# Passport Loyalty Interface Settings (must match working demo)
VENDOR_NAME = "DemoLoyalty"
VENDOR_VER = "1.0"
IFACE_VER = "1.0"

# Reward IDs
POINTS_REWARD_ID = "DEMO-1OFF"      # Points redemption reward ID
PUNCH_REWARD_ID = "PUNCH-FREE"      # Punch card reward ID prefix
PROMO_REWARD_PREFIX = "PROMO"       # Promotion reward ID prefix

# Receipt Labels
RECEIPT_SHORT = "$1OFF"             # <= 8 chars for receipt
RECEIPT_LONG = "Loyalty $ Off"      # <= 24 chars for receipt
...
# CONCURRENCY CONTROL
# =============================================================================
# Ensures only one POS connection is actively processing at a time.
# Prevents global state from being overwritten by overlapping connections.
POS_SINGLETON_LOCK = threading.Lock()

# =============================================================================
# SESSION STATE (protected by POS_SINGLETON_LOCK)
# =============================================================================

current_customer = None             # Customer data from backend
last_promotions_applied = []        # Promotions applied in GetRewards
last_punch_cards = []               # Punch cards that triggered rewards
last_punches_to_record = []         # Items eligible for punch recording
...
def pack_header(xml_bytes: bytes, action: int = ACTION_MESSAGE) -> bytes:
    """Pack POSLOYALTY protocol header."""
...
def parse_header(hdr: bytes):
    """Parse POSLOYALTY protocol header."""
    if len(hdr) != 28:
...
def get_req_ids(root: ET.Element):
    """Extract sequence IDs from request."""
...
def resp_header(pos_seq: str, loy_seq: str) -> str:
    """Build standard response header XML."""
    return (
        f"<ResponseHeader>"
...
def normalize_upc(upc: str) -> str:
    """Preserve UPC exactly as-is (no padding, keeps leading zeros)."""
    if not upc:
        return ""
    return upc.strip()
...
def clear_session_state() -> None:
    """Clear all session state variables. Called on transaction end or connection close."""
...
# =============================================================================
# RECEIPT TEXT HELPERS (spec compliance)
# =============================================================================

def receipt_short(s: str) -> str:
    """Limit text to 8 chars for RewardReceiptDescShort."""
    return (s or "").strip()[:8]


def receipt_long(s: str) -> str:
    """Limit text to 24 chars for RewardReceiptDescLong."""
    return (s or "").strip()[:24]


def receipt_line(s: str) -> str:
    """Limit receipt line to 40 printable chars."""
...
def send_heartbeat(pos_ip: str) -> None:
    """
    Send heartbeat to backend to confirm POS connectivity.
    
    IMPORTANT: Only called when GetLoyaltyOnlineStatusRequest is received.
    This ensures the backend only shows "online" when true end-to-end
    connectivity is confirmed (POS → Edge Agent → Backend).
    """
    try:
        payload = {
...
def extract_line_items(root: ET.Element) -> list:
    ...
    Returns list of dicts with: line_no, upc, description, quantity, amount, price, etc.
    """
    items = []
...
def detect_loyalty_tender(root: ET.Element, reward_id: str) -> float:
    """Detect how much loyalty tender the POS actually applied."""
    dollars = 0.0
    for tline in root.findall(".//TransactionDetailGroup/TransactionLine"):
...
def get_current_unit_price(item: dict) -> float:
    """Get effective per-unit price for an item."""
...
def evaluate_promotions(items: list) -> list:
    """Call backend to evaluate promotional discounts."""
    if not items:
    ...
    try:
        ...
        payload = {"pdiStoreNumber": PDI_STORE_NUMBER, "items": api_items}
...
def build_promotion_rewards_xml(items: list, promotions: list) -> tuple:
    """
    Convert backend promotions into Passport AddReward XML.
    Returns: (list of XML strings, list of applied promotions, set of promo line numbers)
    """
    if not promotions:
        return [], [], set()
...
def build_punch_rewards_xml(punch_cards: list, eligible_items: list) -> list:
    """
    Build FREE ITEM punch rewards XML.
    Applies to the cheapest eligible item per triggered punch card.
    """
    punch_rewards_xml = []
    ...
    for pc in punch_cards:
        ...
        # Use percentOff with 1.0000 (100%) for free item - more spec-compliant than newPrice=0
...
def build_get_rewards_response(root: ET.Element) -> str:
    """
    Main rewards logic with OPTION A:
    
    1. Look up customer
    2. Evaluate promotions for ALL items
    3. Apply promotions (tracks which line numbers got promo discounts)
    4. OPTION A: Filter out promo lines from punch card consideration
    5. Evaluate punch cards ONLY for non-promo items
    6. Apply punch rewards ONLY to non-promo items
    7. Calculate points redemption
    ...
    # Guest mode: evaluate promos only
...
<GetRewardsResponse>
  ...
  <RewardActions/>
    ...
    # =========================================================================
    # STEP 1: Evaluate and apply PROMOTIONS (on ALL items)
    # =========================================================================
    promotions = evaluate_promotions(items)
    ...
    # =========================================================================
    # STEP 2: OPTION A - Filter items for punch cards
    # Items that got a promo discount are EXCLUDED from:
    #   - Earning punches
    #   - Having punch rewards redeemed on them
    # =========================================================================
    ...
    # =========================================================================
    # STEP 3: Evaluate PUNCH CARDS (only for non-promo items)
    # =========================================================================
    if customer_id and punch_eligible_items:
        ...
        if punch_cards_data:
            ...
            # Build punch rewards (only for non-promo items)
            if last_punch_cards:
        ...
        try:
            ...
            if rr.status_code == 200:
                ...
                if recommended > 0:
                    ...
                    # InstantRewardFlag="yes" for auto-apply (no cashier prompt)
                    points_reward_xml = f"""
    <AddReward>
...
def build_finalize_response(root: ET.Element) -> str:
    """Finalize transaction: record points, punches, and redemptions."""
    global current_customer, last_promotions_applied, last_punch_cards, last_punches_to_record
    ...
    # Generate stable transaction ID ONCE and use it everywhere
    ...
    if current_customer:
        try:
            payload = {
                ...
                "transactionId": safe_txn_id,  # Use safe_txn_id consistently
                "pdiStoreNumber": PDI_STORE_NUMBER,
                "lineItems": items,
        ...
        # Record punches (ONLY for punch-eligible items - OPTION A enforced)
        if last_punches_to_record:
            try:
                punch_result = record_punches(
                    current_customer.get("customerId"),
                    last_punches_to_record,
                    safe_txn_id,  # Use same transaction ID
                )
    ...
    # Clear session state after finalize
    clear_session_state()

    # Apply receipt line limits (40 chars max per line)
...
# =============================================================================
# CONNECTION HANDLER (with singleton lock)
# =============================================================================

def handle_client(conn: socket.socket, addr) -> None:
    """
    Handle a single POS connection.
    Uses POS_SINGLETON_LOCK to ensure only one connection is processed at a time.
    Clears session state on exit to prevent dirty state leaking to next connection.
    """
    peer = f"{addr[0]}:{addr[1]}"

    # Acquire lock - only one POS connection can be active at a time
    with POS_SINGLETON_LOCK:
        ...
        try:
            ...
            # CRITICAL: Always clear session state on connection close
            # This prevents dirty state from affecting the next connection
            clear_session_state()
            try:
                conn.close()
            except Exception:
